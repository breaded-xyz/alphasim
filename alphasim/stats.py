...
def calc_stats(result: pd.DataFrame) -> pd.DataFrame:
    ...
    years = len(sum_df) / const.TRADING_DAYS_YEAR
    ...
    df["ann_volatility"] = ret_df.std() * np.sqrt(const.TRADING_DAYS_YEAR)
...
def calc_rolling_ann_vola(result: pd.DataFrame) -> pd.DataFrame:
    pnl = _rollup_equity(result)
    pnl = pnl.pct_change()
    return pnl.ewm(alpha=const.EWMA_ALPHA).std() * np.sqrt(const.TRADING_DAYS_YEAR)
